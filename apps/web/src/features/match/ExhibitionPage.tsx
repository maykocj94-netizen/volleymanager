...
import { FastForward, Play, RefreshCw, RotateCcw, Trophy, Users2 } from "lucide-react";
import {
...
import { useClubAthletes, useMe, useMyClub, useReroll, useScenario, useStartMatch } from "@/lib/game";
...
function tierTone(tier: string) {
  ...
  return "text-red-400"; // muito_dificil / brutal
}
...
function deriveScore(events: MatchEvent[]) {
  ...
  for (const e of events) {
    if (e.event_type === "set_start") {
      home = 0;
      away = 0;
    } else if (e.event_type === "point") {
      if (e.side === "home") home++;
      else away++;
    } else if (e.event_type === "set_end") {
      if (home > away) setHome++;
      else setAway++;
    }
  }
...
export function ExhibitionPage() {
  ...
  const startMatch = useStartMatch(club?.id);
  ...
  async function start() {
    ...
    try {
      const res = await startMatch.mutateAsync({ kind, sex: sexParam, home_tactic: homeTactic });
  ...
  const winnerName =
    status === "done" && result ? (result.winner === "home" ? homeName : "CPU Rivais") : null;
  const running = status === "loading" || status === "live";
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <header>
        <h1 className="text-2xl font-bold">Partida ao vivo</h1>
        <p className="text-sm text-ink-muted">
          Use seu elenco. O adversário (clima, tática e dificuldade) é fixo até você trocar de cenário.
        </p>
      </header>
      ...
      <Card>
        ...
        {/* Adversário da vez (CPU procedural) */}
        <div className="mb-4 rounded-lg border border-graphite-border bg-graphite/40 px-4 py-3">
          ...
          <div className="mt-3">
            ...
            >
              ...
              Mudar cenário{" "}
              {freeLeft > 0
                ? `(${freeLeft} grátis esta semana)`
                : `(${SCENARIO_REROLL_COST} prata)`}
            </Button>
            {reroll.isError && (
              <span className="ml-3 text-sm text-red-400">
                Sem prata suficiente para trocar de novo.
              </span>
            )}
          </div>
        </div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <Button
            ...
            disabled={running || !myAthletes.length || !scenario}
            size="lg"
          >
          ...
          {!myAthletes.length && (
            <span className="text-sm text-amber-400">
              Sem atletas {SEX_LABEL[sex].toLowerCase()} de {kind === "beach" ? "praia" : "quadra"} no elenco.
            </span>
          )}
          {status === "error" && (
            <span className="text-sm text-red-400">Sem conexão com a API.</span>
          )}
        </div>
      ...
      <Card>
        ...
        {winnerName && (
          <p className="mt-4 flex items-center justify-center gap-2 font-semibold text-brand">
